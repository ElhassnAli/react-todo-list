...
export default function Panel({ tasks, onSetTasks, onClear }) {
  return (
    <section className="flex   flex-col flex-1 gap-3 rounded-2xl border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),rgba(255,255,255,0.01))] p-4 md:p-5">
      <Brand />
...
function Brand() {
  return (
    <div className="flex items-center gap-3">
      <div className="grid h-14 w-14 place-items-center rounded-[14px] bg-linear-to-br from-violet-600 to-cyan-500 text-lg font-bold shadow-[0_6px_18px_rgba(8,10,35,0.6)]">
        TD
      </div>
      <div>
...
function InputRow({ tasks, onSetTasks }) {
  ...
  return (
    <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
      <input
        className="w-full rounded-xl border border-white/10 bg-transparent px-3.5 py-3 text-sm text-inherit outline-none shadow-[inset_0_-6px_18px_rgba(0,0,0,0.35)] transition-transform duration-150 focus:-translate-y-0.5 focus:shadow-glow sm:flex-1"
      ...
      />
      <button
        className="w-full rounded-xl border-none bg-linear-to-b from-violet-600 to-cyan-500 px-3.5 py-2.5 font-semibold text-white shadow-[0_8px_24px_rgba(12,10,40,0.6)] transition-transform duration-150 active:translate-y-px active:scale-[0.995] sm:min-w-21.5 sm:w-auto"
        onClick={handelInput}
      >
        Add
      </button>
